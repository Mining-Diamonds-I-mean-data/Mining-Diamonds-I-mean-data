...
def work(sample, api_key, index, list_of_package_total):
    print(f"{bcolors.OKCYAN}Collecting data for:{bcolors.ENDC}", sample, f"{bcolors.FAIL} api key using:{bcolors.ENDC}", api_key, f"{bcolors.OKGREEN}Progress:{bcolors.ENDC}", '{:.2%} ({}/{})'.format(index / list_of_package_total, index, list_of_package_total))
    try:
